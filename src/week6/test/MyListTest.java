...
import week6.prep.MyPrevLinkedList;
...
public class MyListTest {
    ...
    public void setupTestLists(){
        if(type.equalsIgnoreCase("array")) {
            ...
            list = new MyPrevLinkedList<>();
            empty = new MyPrevLinkedList<>();
        }
    ...
    public void testSizeAfterAdd(){
        ...
        empty.add(4);
        empty.add(5);
        Assertions.assertEquals(5, empty.size(),
                "Size after adding 5 elements should be 5");
        Assertions.assertTrue(empty.add(6),
    ...
    public void testGetAfterAdd(){
        for(int i = 0; i<expected.size(); i++){
            empty.add(expected.get(i));
        }
        compareList(expected, empty, "Error with get method");
    }
    @Nested
