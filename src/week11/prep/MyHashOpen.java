...
public class MyHashOpen<K,V> implements Map<K, V> {
    private static final int INITIAL_CAPACITY = 11;
    @SuppressWarnings("unchecked")
    private Map.Entry<K, V>[] table;
    private LinkedList<Map.Entry<K,V>>[] tableChain;


    //Used to figure what to reallocate
    private static final double LOAD_THRESHOLD = 0.75;
    //tracks number of value entries
    private int numKeys;
    //tracks number of deleted entries
    private int numDeletes;
    //placeholder for deleted entries
    private final Map.Entry<K, V> DELETED = new AbstractMap.SimpleEntry<>(null, null);

    public MyHashOpen(){
        table = new Map.Entry[INITIAL_CAPACITY];
        tableChain = new LinkedList[10];
    }

    /**
     * Helper method to find where an entry with the given
     * key would be considering collisions and chaining
     * @param key
     * @return
     */
    ...
    public V get(Object key) {
        //Need the index of key
        int testIndex = key.hashCode() % table.length;
        if(testIndex < 0){
            testIndex += table.length;
        }
        int index = -1;

        //Need to find the location for out key
